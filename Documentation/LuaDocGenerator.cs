...
public static class LuaDocGenerator
{
    ...
    public static void GenerateLuaCATSFiles()
    {
        if (Directory.Exists(LuaCATSOutDir))
            Directory.Delete(LuaCATSOutDir, true);
        ...
        foreach (LuaApiInfo apiInfo in GetAllApiInfo())
        {
            ...
            bool isNewFile = !File.Exists(classFilePath);
            using var sw = new StreamWriter(classFilePath, true, new UTF8Encoding(false));

            if (isNewFile)
            {
                sw.WriteLine("--- This file was auto-generated.");
                sw.WriteLine("--- Do not edit this file directly.");
                sw.WriteLine();
                sw.WriteLine("---@meta");
                sw.WriteLine();
            }
